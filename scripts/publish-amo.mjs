...
Environment
  ...
  AMO_PACKAGE               package to submit
                            (default: release/linkedin-feed-blocker-firefox-${version}.zip)
  AMO_SOURCE                source archive to attach
                            (default: release/linkedin-feed-blocker-source-${version}.zip)
...
const packagePath = path.resolve(
  ...
  process.env.AMO_PACKAGE ??
    `release/linkedin-feed-blocker-firefox-${version}.zip`,
)
const sourcePath = path.resolve(
  root,
  process.env.AMO_SOURCE ??
    `release/linkedin-feed-blocker-source-${version}.zip`,
)
...
// this a sync of five screenshots can never finish in one run. The token is
...
const request = async (method, endpoint, { json, form } = {}) => {
  ...
  for (let attempt = 1; ; attempt += 1) {
    ...
    const response = await fetch(`${API}${endpoint}`, {
      method,
      headers,
      // Both are undefined on the GET call sites, which oxlint cannot see
      // because it does not follow `method` back to the callers.
...
const applyListingAssets = async remotePreviews => {
  ...
  // image plus the deletes means this listing's five screenshots need more than
  // an hour's budget on their own. Crossing the hourly boundary has been seen
  // to cost a single wait of just under an hour, so saying this up front is the
