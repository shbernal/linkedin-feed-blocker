...
- There are no fixture tests, unit tests, or real-browser smoke tests.
...
2. Route-gate selector application so `/feed/` and `/mynetwork/grow/` only query
...
5. Add tests for settings normalization, popup persistence, background command
   routing, and content-script hide/restore behavior.
